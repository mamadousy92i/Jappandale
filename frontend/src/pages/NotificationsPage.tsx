  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-14 sm:px-6 sm:py-20">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-semibold tracking-[4px] text-gold-dark uppercase">{t("notifications.eyebrow")}</p>
          <h1 className="mt-2 font-heading text-3xl font-bold text-ink sm:text-4xl">{t("notifications.title")}</h1>
          <p className="mt-2 text-sm text-ink-secondary">{t("notifications.intro")}</p>
        </div>
        {hasUnread && (
      ...
      {loading ? (
        ...
        <div className="mt-9 rounded-2xl border border-red-100 bg-red-50 p-6"><p className="text-sm text-red-700">{t("notifications.loadError")}</p><Button onClick={load} variant="outline" size="sm" className="mt-4 rounded-full"><RefreshCw className="size-3.5" />{t("notifications.retry")}</Button></div>
      ) : notifications.length === 0 ? (
        <div className="mt-9 rounded-[24px] border border-dashed border-black/10 bg-surface-alt px-6 py-14 text-center"><Bell className="mx-auto size-8 text-gold-dark" /><h2 className="mt-4 font-heading text-xl font-bold text-ink">{t("notifications.emptyTitle")}</h2><p className="mt-2 text-sm text-ink-secondary">{t("notifications.emptyText")}</p></div>
      ) : (
